...
const TableProduct = () => {
  ...
  const [currentItem, setCurrentItem] = useState({
    thumbnail: [{ link: "" }],
    variants: [{ quantity: 0, color: { value: "" }, size: { value: "" } }],
  } as any);
  ...
  const updateStatus = async () => {
    setLoading(true);
    const payload =
      currentItem?.status?.value === "active" ? "inactive" : "active";
    ...
    <div className="flex justify-center items-start gap-4">
              ...
              return (
                ...
                >
                  ...
                  <TableCell className="flex items-center gap-4">
                    <Image
                      src={renderImage(item?.thumbnail[0]?.link)}
                      size="mini"
                      className="rounded-lg"
                    />
                    {item?.name}
                  </TableCell>
                  <TableCell>{item?.brand?.brand_name}</TableCell>
                  ...
                  <TableCell>
                    <Button
                      color={`${
                        item?.status?.value === "active" ? "teal" : "grey"
                      }`}
      ...
      <div>
        <Card className="!mt-0">
          <Image
            size="tiny"
            src={renderImage(currentItem?.thumbnail[0]?.link)}
            wrapped
            ui={false}
          ...
          <CardContent>
            ...
            <CardDescription className="!text-[12px]">
              ...
              {currentItem?.variants[0]?.quantity}
            </CardDescription>
            <CardDescription className="!text-[12px]">
            ...
            <CardDescription className="!text-[12px]">
              <strong>Color: </strong>
              {currentItem?.variants[0]?.color?.value}
            </CardDescription>
            <CardDescription className="!text-[12px]">
              <strong>Size: </strong>
              {currentItem?.variants[0]?.size?.value}
            </CardDescription>
            <div className="w-full grid grid-cols-4 gap-2 mt-2">
              <Button
                color={`${currentItem?.bestSeller ? "teal" : "grey"}`}
                onClick={handleOpenModalCreate}
                className="!w-full !m-0 !py-1 !px-0 !text-[12px]"
              >
                B.Seller
              </Button>
              <Button
                color={`${currentItem?.topDeal ? "teal" : "grey"}`}
              ...
              <Button
                color={`${currentItem?.popular ? "teal" : "grey"}`}
                onClick={handleOpenModalCreate}
                className="!w-full !m-0 !py-1 !px-0 !text-[12px]"
              >
                Popular
              ...
              <Button
                color={`${currentItem?.official ? "teal" : "grey"}`}
                onClick={handleOpenModalUpdate}
                className="!w-full !m-0 !py-1 !px-0 !text-[12px]"
              >
                Official
          ...
          <CardContent extra>
            <a className="flex justify-between items-center">
              <Button onClick={updateStatus} loading={loading ? true : false}>
                Change Status
              </Button>
              <Label
                as="a"
