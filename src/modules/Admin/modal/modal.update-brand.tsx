...
const ModalUpdateBrand: React.FC<ModalUpdateBrandProps> = ({ open, setOpen, currentItem, setCurrentItem, initialData }) => {
    ...
    const submit = async () => {
        if (!validate()) {
            ...
            const payload = {
                name: currentItem?.brand_name,
                thumbnail: imageCloud === '' ? currentItem?.brand_thumbnail : imageCloud,
            }
            const res = await BrandService.updateBrand(currentItem?.brand_id, payload)
            if (res?.result) {
                initialData()
                setImageCloud('')
                setOpen(false)
                ...
                setMessage('Update brand failed')
            }
        }
    }
            ...
            <ModalContent image className='!relative !flex !flex-col !justify-center !items-center !gap-8'>
                {
                    loading
                        ...
                        :
                        <Image size='small' src={currentItem?.brand_thumbnail} className='rounded-lg' />
                }
                <Input label='Name' value={currentItem?.brand_name} onChange={(e) => handleChangeCurrentItem('brand_name', e.target.value)} />
                    ...
                    <div className="flex flex-col items-center justify-center">
                        ...
                        <input
                            ...
                            onChange={(e: any) => {
                                ...
                                reader.onloadend = () => {
                                    handleChangeCurrentItem('brand_thumbnail', reader.result as string)
                                }
